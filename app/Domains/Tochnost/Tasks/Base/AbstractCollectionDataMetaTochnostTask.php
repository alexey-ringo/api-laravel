...
class AbstractCollectionDataMetaTochnostTask extends AbstractTochnostTask
{
    ...
    public function run(SearchOrganizationsTochnostRequestDto $dto): ResourceResponseDto
    {
        ...
        $data['data'] = $responseDataDto;
        $data['links'] = (isset($responseJson['links']) && ! empty($responseJson['links']))
            ? (new SearchOrganizationsTochnostResponseLinksDto($responseJson['links']))->toArray() : [];
        $data['meta'] = (isset($responseJson['meta']) && ! empty($responseJson['meta']))
            ? (new SearchOrganizationsTochnostResponseMetaDto($responseJson['meta']))->toArray() : [];

        $responseDto = new SearchOrganizationsTochnostResponseDto($data);
